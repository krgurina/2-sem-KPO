...
# define TEST_SUM	TEST_CREATE_01	 + \
					TEST_CREATE_02	 + \
					TEST_ADDENTRY_03 + \
					TEST_ADDENTRY_04 + \
					TEST_GETENTRY_05 + \
...
int main() {
	...
	try {
		...
		Instance k8 = Create((char*)"TEST", 5);
		Entry k8_e = { 1,"aaaaaaaaa" }, k8_upd_e = { 1,"zzzzzzzz" };
		AddEntry(k8, k8_e);
		UpdEntry(k8, 1, k8_upd_e);
#endif
#ifdef TEST_DICTIONARY
		Instance d1 = Create((char*)"Преподаватели", 5);
		Entry e1 = { 1,"Барковский" }, e2 = { 2,"Жиляк" }, e3 = { 3,"Наркевич" }, e4 = { 4, "Белодед" }, e5 = { 5, "Гринюк" };
		AddEntry(d1, e1);
		AddEntry(d1, e2);
		AddEntry(d1, e3);
		AddEntry(d1, e4);
