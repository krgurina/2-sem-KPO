...
namespace Dictionary {
	...
	Instance Create(char name[DICTNAMEMAXSIZE], int size) {
		if (strlen(name) > DICTNAMEMAXSIZE) throw (char*)THROW01;// превышен размер имени
		if (size > DICTMAXSIZE) throw(char*)THROW02;// превышена ёмкость
		Instance newDict;
	...
	void UpdEntry(Instance& inst, int id, Entry new_ed) {	//изменить объект словаря
